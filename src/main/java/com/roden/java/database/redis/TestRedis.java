...
public class TestRedis {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        redisService.set(("user-" + user.getId()).getBytes(), SerializeUtil.serialize(user));

        byte[] uuu = redisService.get(("user-" + user.getId()).getBytes());

        User u = (User) SerializeUtil.unserialize(uuu);
        System.out.println(u.getId() + u.getName() + u.getDesc());
